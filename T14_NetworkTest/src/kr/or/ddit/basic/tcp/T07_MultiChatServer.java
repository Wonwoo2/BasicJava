...
public class T07_MultiChatServer {
	...
	// 대화방 즉, Mapdp 저장된 전체 유저에게 메시지를 전송하는 메서드
	...
	// 서버에서 클라이언트로 메시지를 전송할 Thread를 Inner 클래스로 정의
	// Inner 클래스에서는 부모 클래스의 멤버들을 직접 사용할 수 있다.
	class ServerReceiver extends Thread {
		...
		public void run() {
			...
			try {
				...
				// 대화명을 받아서 다른 모든 클라이언트에게 대화명 참여 메시지를 보낸다.
