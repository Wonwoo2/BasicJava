...
public class DomParsingTest {
	/*
	 *  The DOM은 W3C(World Wide Web Consortium)표준이다.
	 *  
	...
	public void parse() {
		try {
			...
			// DOM 파서로부터 입력 받은 파일을 파싱하도록 요청(DOM Document object를 리턴함)
			...
			// 속성 접근방법 2 : 노드 객체의 getAttributes() 메서드 이용(속성 노드를 접근하기)
			...
			// 엔터키에 해당하는 부분이 읽힐 수 있으므로, getChildNodes() 보다는 getElementsByTagName()을 이용해 접근하는 게 좋다.
