...
public class RemoteClient {
	public static void main(String[] args) {
		...
		try {
			...
			// 사용할 객체를 불러온다.
			Registry reg = LocateRegistry.getRegistry("192.168.201.41", 6789);
			...
			files[0] = new File("e:/D_Other/Tulip.jpg");
			files[1] = new File("e:/D_Other/Chart.png");
			
			FileInfoVO[] fInfos = new FileInfoVO[files.length];
			
			// 2개의 파일을 읽어서 byte[]에 담아서 서버측 메서드에 전송하면 된다.
			FileInputStream fis = null;
			for (int i = 0; i < files.length; i ++) {
				int len = (int) files[i].length();		// 파일 크기
				...
				fInfos[i].setFileNmae(files[i].getName());		// 파일명 저장
 			}
			
			clientInf.setFiles(fInfos);		// 서버의 파일 저장 메서드 호출
