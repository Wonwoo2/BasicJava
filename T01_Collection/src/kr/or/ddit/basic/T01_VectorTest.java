...
public class T01_VectorTest {
	public static void main(String[] args) {
		...
		List v1 = new Vector<>();	// 리스트의 모든 속성을 갖는다
		...
		// v1.addElement("ccc"); 리스트 타입의 메서드가 아니기 때문에 에러남
		...
		//				=> 데이터가 삭제되면 index번째 다음번쨰 이후의 데이터들이 앞으로 자동으로 당겨져서 채워진다.
		//				=> 반환값 : 삭제된 데이터
		
		// remove(삭제할데이터) => '삭제할 데이터'를 찾아 삭제한다.
		//					=> 만약 '삭제할 데이터'가 여러개이면 앞에서부터 삭제한다.
		//					=> 삭제할 데이터가 '정수형'이거나 'char형'일 경우에는
		//					=> 삭제할 데이터를 객체로 변환해서 사용해야 한다. (아니면 인덱스로 인식함.)
		...
		/*
		 *  제너릭 타입(generic type)
		 *   => Collection객체를 선언할 때 <> 안에 그Collection이 저장할
		 *  	데이터 타입을 정해주는 것을 말한다.
		 *   => 이런 식으로 선언하게 되면 그 테이터 타입 이외의 데이터를 저장할 수 없다.
		...
		// remeveAll(Collection객체) => 벡터의 데이터 중에서
		 ...
		 *  => 반복할 때마다 '배열변수나 Collection계열 변수'의 데이터를 하나씩
		...
		v.setSize(7); // 현재용량 설정 사이즈보다 작으면 신규용량 = 현재용량 * 2
					  // 그래도 설정사이즈 보다 작다면 신규용량 = 설정사이즈로 설정된다.
