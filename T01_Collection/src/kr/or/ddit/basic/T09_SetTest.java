...
public class T09_SetTest {
	...
	public static void main(String[] args) {
		...
		// Set은 데이터의 순서가 없고, 중복을 허용하지 앟는다
		// 그래서 이미 있는 데이터를 add하면 false를 반환하고, 데이터는 추가되지 않는다
		boolean isAdd = hs1.add("F");
		...
		// 1 ~ 100 사이의 중복되지 않은 정수 5개 만들기
		
		Set<Integer> intRnd = new HashSet<>();
		while(intRnd.size() < 5) {	// Set의 데이터가 5개가 될때까지 반복한다.
			int num = (int)(Math.random() * 100 + 1);	// 1부터 100사이의 난수
			intRnd.add(num);
		}
		
		System.out.println("만들어진 난수들 : " + intRnd);
		
		// Collections 유형의 객체들은 서로 다른 자료 구조로 쉽게 변경해서 사용할 수 있다.
