...
public class T14_ThreadSharedDataTest {
	 ...
	 *  4. 각각의 쓰레드는 이 인스턴스의 참조값을 저장한 변경을 이용하여
	 *  	공통 데이터를 사용한다.
	 *  
	 *  예) 원주율을 계산하는 쓰레드가 있고,
...
class ShareData {
	...
	/*
	 *  volatile => 선언된 변수를 컴파일러의 최적화 대상에서 제외 시킨다.
	 *  			즉, 값이 변경되는 즉시 변수에 적용시킨다.
	 *  			다중 쓰레드에서 하나의 변수가 완벽하게 한번에 작동되도록
	 *  			보장하는 키워드(일종의 동기화)
	 */
	volatile public boolean isOk = false;		// 원주율 계산이 완료되었는지 나타내는 변수
}
...
class CalcPIThread extends Thread {
	...
	public void run() {
		...
		for (int i = 1; i <= 150000000; i += 2) {
			if(((i / 2) % 2) == 0) {	// 2로 나눈 몫이 짝수이면(2로 나누었을 때 나머지가 0) +
				sum += (1.0 / i);
			} else {
				sum -= (1.0 / i);
			}
		}
		sd.result = sum * 4;	// 계산된 원주율을 공통 객체의 멤버변수에 저장한다.
