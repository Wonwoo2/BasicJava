...
class ThreadStopEx2 extends Thread {
	...
	public void run() {
		...
		// 방법2 => interrup() 메서드가 호출되었는지 검사하기
		while(true) {
			System.out.println("쓰레드 처리 중");

		
		// 검사 방법1 => 쓰레드의 인스턴스 객체용 메서드를 이용하는 방법
		if(this.isInterrupted()) { // interrupt() 메서드가 호출되면 true
			System.out.println("인스턴스용 isInterrupted()");
			break;
		}
		
		// 검사 방법2 => 쓰레드의 정적 메서드를 이용하는 방법
		/*if(Thread.interrupted()) {	// interrupted() 메서드가 호출되먄 true
