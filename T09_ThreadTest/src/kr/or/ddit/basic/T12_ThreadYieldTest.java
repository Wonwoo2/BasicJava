...
public class T12_ThreadYieldTest {
	 ...
	 *  2. 현재 실행중인 스레드의 상태를 Runnable상태로 바꾼다. (Wating 이나 Blocked상태로 바뀌지 않는다.)
	 *  3. yield() 메서드를 실행한다고 해서 현재 실행중인 쓰레드가 곧바로 Runnable상태로 전이된다고 확신할 수 없다.
	 */
...
class ThreadYield extends Thread {
	...
	public ThreadYield(String name) {
		// 쓰레드는 기본적으로 name속성이 있고, Thread생성자 중에서는 
		// name값을 매개변수로 받아서 실행하는 생성자가 있다.
