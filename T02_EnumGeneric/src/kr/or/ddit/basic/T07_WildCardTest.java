...
public class T07_WildCardTest {
	 ...
	 *  
	 *  와일드 카드와 제한된 타입 파라미터 예시
	 *  1. 동일한 타입 파라미터 타입으로 강제하고 싶은 경우
	 ...
	 *  	ex) public void print(List<? super Integer> list)	// 가능한 예시
	 *  		public <T super Integer> void print(List<T> list) // 불가능한 예시(컴파일 에러)
	 */
	...
	public static void main(String[] args) {
		...
		Juicer.makeJuice(appleBox);// 제너릭메서드나 와일드카드 이용하여 해결가능

		FruitBox<Drink> drinkBox = new FruitBox<>();
		drinkBox.add(new Drink("콜라"));
		drinkBox.add(new Drink("사이다"));
		drinkBox.add(new Drink("환타"));
		drinkBox.add(new Drink("밀키스"));
		drinkBox.add(new Drink("펩시콜라"));
		
		// Juicer.makeJuice(drinkBox);	// Fruit만 올수 있도록 제한 했기 때문에 컴파일 에러
...
class Juicer {
	...
	// static <T> void makeJuice(FruitBox<T> box) {	// 제너릭 메서드 선언하여 처리함.
													// => 타입 제한이 없으므로 모든 타입 가능
