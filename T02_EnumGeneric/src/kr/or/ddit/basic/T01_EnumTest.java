...
public class T01_EnumTest {
	...
	// enum을 사용하면 타입의 안전한 코드가 가능하다(타입 체크를 통한 비교가 가능하기 때문)
	// 타입이 안전하다는 것은 변수의 값과 타입이 동일해야한다는 것이다
	// 타입이 안전하지 않다는 것은 변수의 값은 동일할 수 있지만 타입이 다를 수도 있다는 것이다
	
	// City 열거형 객체 선언 (기본값을 가용하는 열거형)
	...
	public static void main(String[] args) {
		 ...
		 *  2. oridinal() => 열거형 상수가 정의된 순서값을 반환한다. (기본적으로 0부터 시작함)
		 *  3. valueOf("열거형 상수이름") => 지정된 열거형에서 '열거형 상수이름'과 
		 *     일치하는 열거형 상수를 반환한다.
		 */
