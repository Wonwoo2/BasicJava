...
public class RemoteServer extends UnicastRemoteObject implements RemoteInterface{
	...
	public static void main(String[] args) {
		try {
			...
			// 2. 구현한 객체를 클라이언트에서 찾을 수 있도록 Registry 객체를 생성해서 등록한다.
			
			// 포트번호를 지정하여 Registry객체 생성(기본포트값 : 1099(
			Registry reg = LocateRegistry.createRegistry(8888);
			
			// Registry서버에 제공하는 객체 등록
			// 형식) Registry객체변수.rebind("객체의 Alias", 등록한 객체변수);
	...
	public int doRemotePrint(String str) throws RemoteException {
		...
		System.out.println("클라이언트에서 보내온 메시지 출력 끝");
		return length;
	}

	@Override
	public void doPrintList(List<String> list) throws RemoteException {
		System.out.println("클라이언트에서 보낸 List 값들 출력");
		for (int i = 0; i < list.size(); i ++) {
			System.out.println((i + 1) + "번 째 : " + list.get(i));
		}
		System.out.println("클러이언트에서 보낸 List 출력 끝");
	}

	@Override
	public void doPrintVo(TestVO vo) throws RemoteException {
		System.out.println("클라이언트에서 보내온 TestVO 객체의 값 출력");
		System.out.println("testId : " + vo.getTestId());
		System.out.println("testNum : " + vo.getTestNum());
		System.out.println("클라이언트에서 보내온 TestVO 객체 출력 끝");
	}

	@Override
	public void setFiles(FileInfoVO[] fileInfos) throws RemoteException {
		FileOutputStream fos = null;
		String dir = "E:/C_Lib/";		// 파일이 저장될 폴더 지정
		System.out.println("파일 저장 시작");
		for (int i = 0; i < fileInfos.length; i ++) {
			try {
				fos = new FileOutputStream(dir + fileInfos[i].getFileNmae());
				
				// 클라이언트에서 전달한 파일데이터(byte[])를 서버쪽에 저장한다.
				fos.write(fileInfos[i].getFileData());
				fos.close();
			} catch (IOException e) {
				e.printStackTrace();
			}
