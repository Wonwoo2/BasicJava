...
public class T15_RadioButtonTest extends Application{

	@Override
	public void start(Stage primaryStage) throws Exception {
		// 라디오 버튼을 묶음으로 처리할 객체 생성
		ToggleGroup group = new ToggleGroup();		// 하나만 선택되도록 하는 객체
		...
		RadioButton rb2 = new RadioButton("CANENDAR");
		...
		group.selectedToggleProperty().addListener(new ChangeListener<Toggle>() {
			...
			public void changed(ObservableValue<? extends Toggle> observable, Toggle oldValue, Toggle newValue) {
				...
				if(group.getSelectedToggle().getUserData() != null) {
					...
					Image img = new Image(getClass().getResourceAsStream("images/"
														+ url + ".jpg"));
					icon.setImage(img);
				}
			}
