...
public class T17_TableViewTest extends Application{

	@Override
	public void start(Stage primaryStage) throws Exception {
		// TableView에 나타낼 데이터 구성하기
		ObservableList<Member> data = FXCollections.observableArrayList(
				new Member("홍길동", "gildong-hong", 22, "1111-1111", "대전"),
				new Member("홍길서", "gilseo-hong", 33, "2222-2222", "서울"),
				new Member("홍길남", "gilnam-hong", 44, "3333-3333", "부산"),
				new Member("홍길북", "gilbuk-hong", 55, "4444-4444", "대구")
		);
		
		BorderPane root = new BorderPane();
		
		// TableView에 데이터를 세팅하기
		TableView<Member> table = new TableView<>(data);
		TableColumn<Member, String> nameCol = new TableColumn<>("이 름");
		TableColumn<Member, String> engNameCol = new TableColumn<>("영어 이름");
		TableColumn<Member, String> korNameCol = new TableColumn<>("한글 이름");
		...
		korNameCol.setStyle("-fx-alignment: CENTER");		// 컬럼 중앙 정렬
		// 해당 컬럼에 나타낼 데이터 연결하기
		// (출력할 객체의 멤버 변수와 출력할 컬럼을  매칭시킨다.)
		...
		nameCol.getColumns().addAll(korNameCol, engNameCol);		// 컬럼 병합하기
		...
		btnAdd.setOnAction(e -> {
			...
			if(!Pattern.matches("^[0-9]+$", txtAge.getText())) {
				errMsg("데이터 오류", "나이는 정수형으로 입력해주세요.");
				txtAge.requestFocus();		// 해당 객체에 Focus 추가
				return;
			}
			...
			infoMsg("작업 결과", txtKorName.getText() + "님 정보를 추가하였습니다.");
		...
		btnEdit.setOnAction(e -> {
			if(txtKorName.getText().isEmpty() 
				...
				infoMsg("작업 결과", txtKorName.getText() + "님 정보를 수정하였습니다.");
		...
		btnTest1.setOnAction(e -> {
			...
			// => setEditable(boolean 값) : 이 메서드에 true를 설정하면 '입력가능' false를 설정하면
			//							   '읽기전용'이 된다.
	...
	public class Member {
		...
		public String toString() {
			return "Member [korName=" + korName + ", engName=" + engName + ", age=" + age + ", tel=" + tel + ", addr="
					+ addr + "]";
		}
	}
}